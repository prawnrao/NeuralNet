...
class ActivationFunction(object):
    ...
    def __repr__(self):
        return """{} \n\t{} \n\t{}\n""".format(self.name, self.func.__doc__, self.dfunc.__doc__)
...
class NeuralNet(object):
    ...
    def __init__(self, input_nodes, hidden_nodes, output_nodes, 
        ...
        self.ih_weights = Matrix.random_matrix(self.hidden_nodes, self.input_nodes)
        self.hidden_bias = Matrix(self.hidden_nodes, 1)
        self.ho_weights = Matrix.random_matrix(self.output_nodes, self.hidden_nodes)
    ...
    def back_propogate(self, inputs, hidden, output, labels):
        """ Propogates and assigns the weights back from the outputs to the inputs"""
        hidden_errors, output_errors = self.errors(output, labels)
        
        output_gradient = output.apply(sigmoid.dfunc)
        output_gradient = self.learning_rate * Matrix.Hadamard(output_errors, output_gradient)
        delta_w_ho = Matrix.matmul(output_gradient, hidden.T)
        
        hidden_gradient = hidden.apply(self.activation_function.dfunc)
        hidden_gradient = self.learning_rate * Matrix.Hadamard(hidden_errors, hidden_gradient)
    ...
    def train(self, inputs, labels, stocastic=True):
        """ Trains the neural net, with the option of stocastic or batch training"""
        #Converting inputs to matrix object
        inputs = Matrix.from_array(inputs)
        hidden, output = self.feed_forward(inputs)
        if stocastic:
            hidden_gradient, delta_w_ih, output_gradient, delta_w_ho = self.back_propogate(inputs, hidden, output, labels)
    ...
    def batch_train(self, inputs_array, label_function=xor_logic, batch_size=100):
